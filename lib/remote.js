...
//
// Smart fallback: if no real server is configured yet (API_CONFIGURED is
// false — e.g. before you deploy), we skip the network entirely and serve the
// bundled list. No failed fetches, no 8s timeouts. Set a real API_BASE and it
// upgrades to the live list on the next refresh, with the same offline safety.

import { ENDPOINTS, API_CONFIGURED } from './config.js';

// --- Config -----------------------------------------------------------------
const API_URL = ENDPOINTS.newsletters;
...
export async function fetchNewsletters({ force = false } = {}) {
  // No real server configured yet → use the bundled list, skip the network.
  if (!API_CONFIGURED) return readBundled();
